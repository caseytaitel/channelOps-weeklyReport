...
The HTML has three tabs: **Overview**, **Unqualified Deal Regs**, **Stalled Deals**. The last two tabs show a row count. Open it in a browser so the tabs work; Slack's file preview may not.

**Overview** is a number summary: open Channel deals, deal regs with a known status, Qualified, then the two review counts (unqualified deal regs and stalled deals). Clicking a review count opens that tab. The same page has the two hygiene reminders (static HubSpot links).

**Unqualified Deal Regs** columns: Deal Name, Days Since Approval, Owner, Channel Partner, Partner Contact Email.

**Stalled Deals** is one table per close-date group (see grouping below). Columns: Deal Name, Stage, Days in Stage, Close Date, Owner, Channel Deal Type, Channel Partner, Partner Contact Email. Grouped tables use the same column widths so they line up when stacked.
...
| `scripts/report_config.py` | Portal IDs, thresholds, fiscal year, HubSpot token, fallback stage labels, static hygiene links |
| `scripts/hubspot_client.py` | Talk to HubSpot (deal search, owner names including archived, stage labels) |
...
The table is then **grouped by HubSpot close date** for display. That does not change who is stalled. Fiscal year starts Feb 1 (`FY_START_MONTH` in `scripts/report_config.py`; Q1 Feb–Apr, Q2 May–Jul, Q3 Aug–Oct, Q4 Nov–Jan). Empty groups are omitted. Order: **Past close date**, remaining quarters of the current fiscal year, next FY (labeled `FYxx`), then **No close date**. A close date that has already passed goes in Past, even if it is in the current quarter. Inside each group, longest in stage first.
...
| `dealstage` | Stalled "Stage" column (label from `/crm/v3/pipelines/deals`; `FALLBACK_STAGE_LABELS` in `scripts/report_config.py` if that call fails) |
| `hs_v2_date_entered_current_stage` | Days-in-stage for Stalled |
| `hubspot_owner_id` | Resolved to a display name via `/crm/v3/owners/{id}` (cached). A 404 is retried with `archived=true` so deactivated reps still get a name. Missing owner → "Unassigned"; lookup failure → "Owner {id}" |
...
- **API pagination:** deal search pages at 200; no server-side sort. Client sorts Unqualified Deal Regs by days since approval descending (missing dates first), Stalled by `days_in_stage` descending, then groups Stalled by close date.
...
- **Display coloring is not the query definition.** Cutoffs are >14 days (deal regs) and >30 days (stalled). Row/badge colors in `scripts/report_html.py` are separate: under 30 uncolored (same-sized number, no fill), yellow 30–59d, orange 60–89d, red 90d+. A missing days-since-approval value shows "—". The Unqualified Deal Regs and Stalled Deals tabs show those bands on the right of the tab bar.
...
A local Windows Task Scheduler job runs `run_weekly_report.bat` (Mondays at 7am on this machine). That bat file is gitignored because it is machine-specific. It `cd`s to this folder, activates `venv`, then runs `python scripts\weekly_report.py`, appending output to `logs\weekly_report.log` (also gitignored).
...
If `HUBSPOT_TOKEN` is missing, the script exits immediately with `ERROR: HUBSPOT_TOKEN environment variable not set.` If HubSpot is unreachable or returns an error, it exits with `ERROR: HubSpot request failed: ...` and does not write a report file.
